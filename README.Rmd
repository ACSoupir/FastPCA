...
First exploring the eigenvalues calculated by all methods, on the high end they are all very similar as expected. FastPCA uses essentially the same method as PCAone uses for `'alg1'` so its logical that PCAone with `'alg1'` produces results much more siilar to FastPCA. Interestingly, FastPCA without random projection and power iterations produces results more similar to `'alg1'` and FastPCA's Randomized method. 

```{r eigenvalue_against, echo = FALSE, fig.width=8, fig.height=8, dpi=300, out.width="75%", fig.align="center"}
library(ggplot2)
